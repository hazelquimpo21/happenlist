...
function DisplayItemCard({ item }: { item: DisplayItem }) {
  ...
  return (
    <div
      className={`
        group relative rounded-xl border border-sand bg-warm-white p-4
    ...
    >
      ...
      {item.actions.length > 0 && (
        <div className="flex items-center gap-2 mt-3 pt-3 border-t border-sand/60">
...
function TimeGroupSection({ group }: { group: TimeGroup }) {
  return (
    <section className="mb-8">
      <h2 className="font-display text-lg font-bold text-charcoal mb-4 sticky top-0 bg-cream/95 backdrop-blur-sm py-2 z-10">
        {group.label}
        <span className="text-stone font-body text-sm font-normal ml-2">
...
function TimelineLoading() {
  return (
    <div className="space-y-8">
      {[1, 2, 3].map((i) => (
        <div key={i}>
          <div className="h-6 w-24 bg-sand rounded animate-pulse mb-4" />
          <div className="space-y-3">
            {[1, 2].map((j) => (
              <div
                key={j}
                className="rounded-xl border border-sand bg-warm-white p-4"
              >
                <div className="h-4 w-32 bg-sand rounded animate-pulse mb-2" />
                <div className="h-4 w-48 bg-sand/60 rounded animate-pulse mb-2" />
                <div className="h-3 w-64 bg-sand/40 rounded animate-pulse" />
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between">
          ...
          <button
            ...
            className="text-sm font-medium text-stone hover:text-charcoal transition-colors px-4 py-2 rounded-lg border border-sand hover:border-charcoal/20"
          >
