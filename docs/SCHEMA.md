...
**Display order on the detail page:** short_description (under title) → good_for pills → happenlist_summary → organizer_description → description. All are optional — sections only render when populated.

**SEO fallback chain:** `meta_description` → `short_description` → `description` (truncated to 155 chars).

#### Chrome extension strategy for descriptions

The extension should populate **three** of the four fields. Leave `happenlist_summary` blank — that's written by staff/AI during admin review.

| Field | What the extension should send | How to extract it |
|-------|-------------------------------|-------------------|
| `organizer_description` | **Always send.** Copy the event's main body text verbatim — preserve line breaks, bullet lists, whatever the source page has. This is the "source of truth" text that admins reference during review. Don't clean it up, don't summarize, don't reformat. | Grab the main content area of the event page. On Eventbrite: `.structured-content-rich-text`. On Facebook: the "Details" section. On custom sites: the largest text block near the title. |
| `short_description` | **Always send.** A single sentence (max 160 chars) that captures the event's elevator pitch. This shows on cards, in search results, and as the SEO fallback. | Option A: Take the first sentence of the organizer text. Option B: If the source page has a subtitle or tagline field, use that. Option C: AI-generate from the organizer_description. Truncate to 160 chars. |
| `description` | **Send when possible.** A cleaned-up version suitable for the "About" section. Strip promotional fluff ("Don't miss out!!!"), fix ALL CAPS, remove repeated info that's already captured in other fields (price, date, location). Keep it factual and readable. | Clean the organizer_description: collapse excessive whitespace, strip emoji spam, remove "Click here to buy tickets" CTAs. If the source text is already clean, this can be identical to organizer_description. |
| `happenlist_summary` | **Do NOT send.** Leave null. | Written by staff/AI during review. |

**Edge cases:**
- If the source page has almost no text (just a title + date), send `organizer_description` as whatever text exists (even one sentence) and skip `description` and `short_description`.
- If the source text is extremely long (>5000 chars), still send it all as `organizer_description`. The UI handles long text with scroll/expand. Truncate `description` to a reasonable ~2000 chars.
- Don't duplicate: if `description` would be identical to `organizer_description`, just send `organizer_description` and leave `description` null. Admins can fill it in during review.
...
**Recommended fields (always try to send these):**

| Field | Type | Notes |
|-------|------|-------|
| `organizer_description` | string | **Priority 1.** Verbatim text from the source page. See [description strategy](#chrome-extension-strategy-for-descriptions). |
| `short_description` | string | **Priority 2.** Max 160 chars. First sentence or tagline. Used on cards + SEO. |
| `description` | string | Cleaned version of organizer_description. Skip if it would be identical. |
| `end_datetime` | string | ISO 8601. Without this, events show "Start time only" in the UI. |
| `price_type` | string | `free` (default), `fixed`, `range`, `varies`, `donation`. See [pricing rules](#pricing-gotchas) below. |
| `price_low` / `price_high` | number | Dollar amounts. Required when `price_type` is `fixed` or `range`. |
| `price_details` | string | Free-text for complex tiers, e.g. `"Early bird $15, General $25, VIP $50"`. Send this whenever pricing isn't a clean number. |
| `category_slug` | string | One of: `music`, `arts-culture`, `family`, `food-drink`, `sports-fitness`, `nightlife`, `community`, `classes-workshops`, `festivals`, `theater-film`, `markets-shopping`, `talks-lectures`, `outdoors-nature`, `charity-fundraising`, `holiday-seasonal`. |

**Optional fields (send when available):**

| Field | Type | Notes |
|-------|------|-------|
| `website_url` | string | The event's own external page (not the source_url — that's the page being scraped). |
| `instagram_url` | string | Instagram page/post for the event. |
| `facebook_url` | string | Facebook event URL. |
| `registration_url` | string | RSVP or signup link (distinct from ticket_url). |
| `ticket_url` | string | Where to buy tickets. Shown as "Get Tickets" button. |
| `age_low` | number | Minimum age, e.g. `21` for a bar show. |
| `age_high` | number | Maximum age, e.g. `12` for a kids event. Must be >= `age_low`. |
| `age_restriction` | string | Human-readable note: `"21+"`, `"All ages"`, `"Ages 6-12"`. Shown as orange badge on cards. |
| `is_family_friendly` | boolean | Shown as green "Family Friendly" badge. Set `true` for parks, outdoor fests, family-oriented things. |
| `good_for` | string[] | Audience tags. Valid slugs: `date_night`, `families_young_kids`, `families_older_kids`, `pet_friendly`, `foodies`, `girls_night`, `guys_night`, `solo_friendly`, `outdoorsy`, `creatives`, `music_lovers`, `active_seniors`, `college_crowd`, `first_timers`. See [good_for details](#good-for-audience-tags). |
| `is_all_day` | boolean | Set `true` for events with no specific start time (festivals, all-day sales). Default: `false`. |
| `timezone` | string | IANA timezone. Default: `America/Chicago`. Only send if the event is in a different timezone. |
| `scraped_data` | object | Raw JSON blob of everything the extension scraped. Useful for debugging. Not shown in UI. |
...
`is_free` is a **generated column** — the API ignores it. Set `price_type` instead.

| Source page says | Send as | Why |
|-----------------|---------|-----|
| "Free" / "No cost" / "$0" | `price_type: "free"` | Default. Don't send price_low/high. |
| "$25" | `price_type: "fixed"`, `price_low: 25` | Single price. `price_high` not needed. |
| "$15 - $50" | `price_type: "range"`, `price_low: 15`, `price_high: 50` | Both required. |
| "Tickets start at $20" | `price_type: "range"`, `price_low: 20` | OK to omit `price_high` if unknown. |
| "Suggested donation $10" | `price_type: "donation"`, `price_low: 10` | `price_low` = suggested amount. |
| "Early bird $15, General $25, VIP $50" | `price_type: "range"`, `price_low: 15`, `price_high: 50`, `price_details: "Early bird $15, General $25, VIP $50"` | Use `price_details` for the human-readable breakdown. |
| "Per session" / "$10/class" | `price_type: "per_session"`, `price_low: 10` | For class/workshop series priced individually. |
| No pricing info found | `price_type: "free"` | Default assumption. Admins fix during review. |

#### Date/time gotchas

- `start_datetime` must be **ISO 8601 with timezone offset**: `2026-02-14T19:00:00-06:00` (CST). Do NOT send bare datetimes like `2026-02-14T19:00:00` — they'll be interpreted as UTC and show the wrong time.
- `instance_date` is auto-derived from `start_datetime` if omitted — you almost never need to send it.
- For multi-day events (festivals), send the **first day** as `start_datetime` and the **last day end** as `end_datetime`. Each day does not need separate events.
- `is_all_day: true` — use for events with no specific start time (all-day festivals, weekend sales). The UI hides the time and just shows the date.
- `timezone` defaults to `America/Chicago`. Only override if the event is actually in a different timezone.

#### Category guidance

One category per event. Pick the **primary draw** — what would someone search for? Use `good_for` tags for the secondary angle.

| Event | Category | Good For tags |
|-------|----------|---------------|
| Jazz brunch at a restaurant | `music` | `foodies`, `date_night` |
| Outdoor yoga class | `sports-fitness` | `outdoorsy`, `solo_friendly` |
| Kids' Halloween parade | `family` | `families_young_kids`, `first_timers` |
| Charity gala with live band | `charity-fundraising` | `date_night`, `music_lovers` |
| Farmers market | `markets-shopping` | `foodies`, `families_older_kids`, `pet_friendly` |

When in doubt, check the [full category list](#current-categories).

#### Location matching

The API tries to match existing venues before creating new ones. To get the best match:

1. **Always send `google_place_id`** if you can get it. This is the most reliable match key. Google Places API or extracting from the source page's embedded map link.
...
Events are deduped by `source_url`. The API returns `409` with the existing event ID if the URL was already scraped. The extension should:
- Check for `409` and show "Already exists" rather than retrying.
- Use the **canonical URL** of the event page (not a shortened/tracking URL) to avoid duplicate submissions of the same event from different link formats.
- `source_url` should be the page the user is looking at, not an API endpoint or embed URL.

#### Fields NOT to send

| Field | Why |
|-------|-----|
| `happenlist_summary` | Written by staff/AI during review |
| `is_free` | Generated column. Set `price_type` instead. |
| `status` | Always set to `pending_review` by the API. |
| `meta_title` / `meta_description` | Auto-generated from title and short_description during review. |
| `image_url` / `thumbnail_url` / `flyer_url` | Use `/api/images/upload` instead. Unless you've already uploaded and have the CDN URL. |
