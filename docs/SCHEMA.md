...
The extension should populate **all four** fields. `happenlist_summary` is AI-generated by the scraper (not written by staff).
...
| `happenlist_summary` | **Send when possible.** AI-generated editorial summary in third person. 2-3 sentences, written as if by a local events editor — engaging but factual. Should highlight what makes this event worth attending, not just repeat the title. | AI-generate from the `organizer_description`. Tone: friendly, third-person, local ("Milwaukee's own…"). Avoid first-person ("Join us…") and promotional language ("Don't miss…"). If the source text is too short to summarize meaningfully (< ~50 chars), skip this field. |

**Edge cases:**
- If the source page has almost no text (just a title + date), send `organizer_description` as whatever text exists (even one sentence) and skip `description`, `short_description`, and `happenlist_summary`.
...
- `happenlist_summary` should never just parrot the title or short_description. If the AI can't add value beyond what's already in those fields, leave it null.
