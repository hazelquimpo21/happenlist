...
CREATE INDEX IF NOT EXISTS idx_events_free
  ON public.events (instance_date, status)
  WHERE is_free = true;
...
-- 📊 PUBLISHED EVENTS: Optimizes public-facing queries
-- Note: We filter by date at query time since CURRENT_DATE is not immutable
...
-- 📊 ENTITY AUDIT: For viewing history of any entity (event, series, etc.)
-- Speeds up: "WHERE entity_id = ?"
